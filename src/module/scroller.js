 ...
 *     scroll region.
 * @param {Provider} source A provider of the content to be
 *     displayed in the infinite scroll region.
 * @param {Object} options
 */
function BetterScroller(scroller, source, options = {}) {
  RUNWAY_ITEMS = options.runwayItems || RUNWAY_ITEMS;
  RUNWAY_ITEMS_OPPOSITE = options.runwayItemsOpposite || RUNWAY_ITEMS_OPPOSITE;
  SCROLL_RUNWAY = options.scrollRunway || SCROLL_RUNWAY;
  ANIMATION_DURATION_MS = options.animationDurationMs || ANIMATION_DURATION_MS;
...
BetterScroller.prototype = {
  ...
  onScroll_: function() {
    ...
    let lastScreenItem = this.calculateAnchoredItem(this.anchorItem, this.scroller_.offsetHeight);
    if (delta < 0) {
      // 向上滚动
   ...
   * TODO: Limit this based on the change in visible items rather than looping over all items.
   * @returns {Array}
   * @private
   */
  collectUnusedNodes_: function() {
    let unusedNodes = [];
  ...
  attachContent: function() {
    let i;
    let unusedNodes = this.collectUnusedNodes_();
    ...
    for (i = this.firstAttachedItem_; i < this.lastAttachedItem_; i++) {
      ...
      let node = this.items_[i].data ? this.source_.render(this.items_[i].data, unusedNodes.pop()) : this.getTombstone();
